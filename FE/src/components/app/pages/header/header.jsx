...
export default function Header(item) {
    ...
    useEffect(() => {
        const fetchData = async () => {
          try {
            const response = await fetch(`${API_FAVORITES_URL}/getAllFavorite.php`);
    ...
    useEffect(() => {
        const fetchData = async () => {
          try {
            const response = await fetch(`${API_STORE_URL}/getAllStore.php`);
            const data = await response.json();
            setStore(data);
          } catch (error) {
            console.error("Error fetching favorite status:", error);
    ...
    useEffect(() => {
        ...
        if (indexFavorite !== -1) {
            fetch(`${API_FAVORITES_URL}/deleteFavorite.php?id=${encodeURIComponent(indexFavorite)}`)
        ...
        if (indexStore !== -1) {
            fetch(`${API_STORE_URL}/deleteStore.php?id=${encodeURIComponent(indexStore)}`)
                .then(response => response.json()) 
                .catch(error => console.error('Error fetching data:', error));
