...
export default function Post() {
        ...
        if (data !== null) {
            const updatedData = data.map((element) => {
                if (element.id === id) {
                    ...
                    fetch(`${API_ADMIN_URL}/setStatus.php?&url=${encodeURIComponent(element.Page)}&variable=${encodeURIComponent(newStatus)}&id=${encodeURIComponent(id)}`)
        ...
        if (Use === "All") {
            ...
            for (let index = 0; index < checkedItems.length; index++) {
                if (checkedItems[index] === true) {
                    try {
                        const response = fetch(`${API_ADMIN_URL}/delete.php?&id=${encodeURIComponent(data[index].id)}`);
    ...
    return (
        <form className="flex w-full h-screen justify-center items-center" action={`${API_ADMIN_URL}/input.php`} method="post" onSubmit={handleSubmit}>   
